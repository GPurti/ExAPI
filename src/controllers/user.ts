...
import { insertUser,getUsers,getUser,updateUser,deleteUser,getUserSubjects} from "../services/user";
...
const getPersonSubject=async({params}:Request,res:Response)=>{
    try{
        const {idUser}=params;
        const response=await getUserSubjects(idUser);
        const data=response ? response:"NOT_FOUND";
        res.send(data);
    } catch(e){
        handleHttp(res,"ERROR_GET_USER");
...
export{getPerson,getPeople,postPerson,updatePerson,deletePerson,getPersonSubject};