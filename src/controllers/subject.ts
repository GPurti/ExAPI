...
import { insertSubject,getSubject,getSubjects,updateSubject,deleteSubject, matriculateSubject,getSubjectUsers} from "../services/subject";
import { getUserSubjects } from "../services/user";
...
const users_Subject = async ({params}:Request,res:Response)=>{
    try{
        const {idSubject } = params;
        console.log(idSubject);
        const response = await getSubjectUsers(idSubject);
        res.send(response);
    }catch(e){
        handleHttp(res,"ERROR_USERS_IN_A_SUBJECT");
    }
};

export{get_Subject,get_Subjects,post_Subject,update_Subject,delete_Subject,matriculate_Subject,users_Subject};